...
- (void)creatButtonsWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructionButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitlesArray
{
    ...
    if (title) {
        ...
        self.LXActionSheetHeight = self.LXActionSheetHeight + 2*TITLE_INTERVAL_HEIGHT + _labelSize.height ;
    ...
    if (destructiveButtonTitle) {
        ...
        if (self.isHadTitle == YES) {
            //当有title时
            [destructiveButton setFrame:CGRectMake(BUTTON_INTERVAL_WIDTH +(self.bounds.size.width - BUTTON_INTERVAL_WIDTH * 2 -10 - MARGIN_WIDTH)/2 +7,self.LXActionSheetHeight, (self.bounds.size.width - BUTTON_INTERVAL_WIDTH * 2 -10 - MARGIN_WIDTH)/2, BUTTON_HEIGHT)];
            ...
            if (otherButtonTitlesArray && otherButtonTitlesArray.count > 0) {
                ...
                [destructiveButton setFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, self.LXActionSheetHeight, ([UIScreen mainScreen].bounds.size.width - BUTTON_INTERVAL_WIDTH * 2  - MARGIN_WIDTH), BUTTON_HEIGHT)];
            ...
            if (otherButtonTitlesArray && otherButtonTitlesArray.count > 0) {
                ...
                [destructiveButton setFrame:CGRectMake(BUTTON_INTERVAL_WIDTH, self.LXActionSheetHeight +5, ([UIScreen mainScreen].bounds.size.width - BUTTON_INTERVAL_WIDTH * 2  - MARGIN_WIDTH), BUTTON_HEIGHT)];
        ...
        if (otherButtonTitlesArray.count > 0) {
            ...
            if (self.isHadTitle == YES || self.isHadDestructionButton == YES) {
                for (int i = 0; i<otherButtonTitlesArray.count; i++) {
                    ...
                    otherButton.tag = self.postionIndexNumber;
                    [otherButton addTarget:self action:@selector(clickOnButtonWith:) forControlEvents:UIControlEventTouchUpInside];
                    [otherButton setFrame:CGRectMake(otherButton.frame.origin.x, self.LXActionSheetHeight, otherButton.frame.size.width - MARGIN_WIDTH, otherButton.frame.size.height)];
...
//        当有title或destructionButton或otherbuttons时
        if (self.isHadTitle == YES || self.isHadDestructionButton == YES || self.isHadOtherButton == YES) {
            [cancelButton setFrame:CGRectMake(cancelButton.frame.origin.x, self.LXActionSheetHeight, (self.bounds.size.width - BUTTON_INTERVAL_WIDTH * 2 -10 - MARGIN_WIDTH)/2, BUTTON_HEIGHT)];
    ...
    //
        [UIView animateWithDuration:ANIMATE_DURATION animations:^{
            [self.backGroundView setFrame:CGRectMake(MARGIN_WIDTH/2, ([UIScreen mainScreen].bounds.size.height-self.LXActionSheetHeight)/2 , [UIScreen mainScreen].bounds.size.width - MARGIN_WIDTH,self.LXActionSheetHeight)];
